...
inline void outpin_on( int pin )
{
    PORT->Group[0].OUTSET.reg = ( 1 << pin );
}
inline void outpin_off( int pin )
{
    PORT->Group[0].OUTCLR.reg = ( 1 << pin );
}
inline void outpin_toggle( int pin )
{
    PORT->Group[0].OUTTGL.reg = ( 1 << pin );
}
...
inline int inpin_get( int pin )
{
    return ( ( PORT->Group[0].IN.reg & ( 1 << pin ) ) != 0 );
}
